...
if($pg == 'search'){
 	...
 	$aa = '';
 	$sql = $db->query("SELECT * FROM ".prefix."families WHERE name LIKE '%{$searsh}%'");
 	$aa .='<ul class="pt-drop">';
	if(!empty($searsh)) {
	 	if($sql->num_rows){
...
elseif($pg == 'sendlanguage'){
	...
	if($_SERVER['REQUEST_METHOD'] === 'POST' && us_level == 6){
		...
		db_update_global('site_language', json_encode($langsec));
		$alert = [
			'type'  =>'success',
	...
	if($_SERVER['REQUEST_METHOD'] === 'POST'){
		if(!us_level){
			...
			if(empty($reg_email)){
				...
				$body    = "";
				$html    = fh_reset_tmp($login, $to, $reset_url);
				$mail    = new Mail($to, $from, $subject, $body, $html);
