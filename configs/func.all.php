...
function get_child($cid, $frt = false){
	...
	if($sql_p->num_rows){
		while($rs_p = $sql_p->fetch_assoc()){
			...
			$list .= '<a rel="item-'.$rs_p['id'].'"'.($rs_p['type']==2?' class="partner"':'').'>
			'.(!$rs_p['death']?'<span class="pt-dead"><i class="fas fa-sad-cry"></i> '.$lang['rip'].'</span>':'').'
				<div class="pt-thumb">
					<img src="'.path.'/'.db_get('members', 'photo', $rs_p['id']).'" onerror="this.src=\''.nophoto.'\'" />
				</div>
				<strong>'.db_get('members', 'firstname', $rs_p['id']).'</strong>';
				if((us_id && $lg == $fam_auth) || (us_id && in_array(us_name, explode(',', $fam_mode)))){
					$list .= '<span class="pt-options">
						<b class="tree-edit" rel="'.$rs_p['id'].'"><i class="fas fa-pencil-alt"></i></b>
						<b class="tree-delete" rel="'.$rs_p['id'].'"><i class="far fa-trash-alt"></i></b>
					</span>';
				}
			$list .= '</a>';
		}
	...
	'.(!db_get('members', 'death', $cid)?'<span class="pt-dead"><i class="fas fa-sad-cry"></i> '.$lang['rip'].'</span>':'').'
		<div class="pt-thumb ">
			<img src="'.path.'/'.db_get('members', 'photo', $cid).'" onerror="this.src=\''.nophoto.'\'" />
		</div>
		<strong>'.db_get('members', 'firstname', $cid).'</strong></div>';
		if((us_id && $lg == $fam_auth) || (us_id && in_array(us_name, explode(',', $fam_mode))) || (us_id && db_get('members', 'user', $cid)==us_name)){
			$list .= '<span class="pt-options">
				<b class="tree-edit" rel="'.$cid.'"><i class="fas fa-pencil-alt"></i></b>';
				if($lg == $fam_auth || (us_id && in_array(us_name, explode(',', $fam_mode)))){
					$list .= '
	...
	if($sql_exp->num_rows){
		while($rs_exp = $sql_exp->fetch_assoc()){
			...
			'.(!$rs_exp['death']?'<span class="pt-dead"><i class="fas fa-sad-cry"></i> '.$lang['rip'].'</span>':'').'
				<div class="pt-thumb">
					<img src="'.path.'/'.db_get('members', 'photo', $rs_exp['id']).'" onerror="this.src=\''.nophoto.'\'" />
				</div>
				<strong>'.db_get('members', 'firstname', $rs_exp['id']).'</strong>';
				if((us_id && $lg == $fam_auth) || (us_id && in_array(us_name, explode(',', $fam_mode)))){
					$list .= '<span class="pt-options">
						<b class="tree-edit" rel="'.$rs_exp['id'].'"><i class="fas fa-pencil-alt"></i></b>
						<b class="tree-delete" rel="'.$rs_exp['id'].'"><i class="far fa-trash-alt"></i></b>
					</span>';
				}
			$list .= '</a>';
		}
	...
	$sql_m = $db->query("SELECT * FROM ".prefix."members WHERE parent = '{$cid}' && type != 2 && type != 3");
